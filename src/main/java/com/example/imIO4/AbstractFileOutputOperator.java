...
public abstract class AbstractFileOutputOperator<INPUT> extends BaseOperator implements Operator.CheckpointListener
{
    ...
    if (status.getLen() != offset) {
      ...
      if (alwaysWriteToTmp) {
        ...
        //is restored to an earlier check-pointed window, it will look for an older tmp.
          LOG.info("filesinfo "+partFileName+"  recF:"+recoveryFileName);
      ...
      if (alwaysWriteToTmp && filesWithOpenStreams.contains(filename)) {
        String currentTmp = filename + '.' + System.currentTimeMillis() + TMP_EXTENSION;
        ...
        streamsCache.put(filename, new FSFilterStreamContext(outputStream));
          LOG.info("fileInfo12 "+filename);
        fileNameToTmpName.put(filename, currentTmp);
      }
      inputStream.close();
    }
  }
      ...
      public FSFilterStreamContext load(@Nonnull String filename)
      {
        ...
        if (!alwaysWriteToTmp) {
          ...
          String tmpFileName = fileNameToTmpName.get(partFileName);
            ...
            LOG.info("tmpfile1 "+tmpFileName+" partF "+partFileName);
          if (tmpFileName == null) {
            tmpFileName = partFileName + '.' + System.currentTimeMillis() + TMP_EXTENSION;
              LOG.info("tmpf12 "+tmpFileName);
  ...
  public void committed(long l)
  {
    if (alwaysWriteToTmp) {
      ...
      try {
        ...
        throw new RuntimeException("failed to commit adi "+e.getMessage(), e);
      }
    }
  }
  ...
  protected void finalizeFile(String fileName) throws IOException
  {
    ...
    Path destPath = new Path(filePath + Path.SEPARATOR + fileName);
      LOG.info("finalizeFile log file:"+fileName+" tmpF:"+tmpFileName+" scr:"+srcPath+" dest:"+destPath);
      LOG.info("mapFileNameToTmp:"+fileNameToTmpName);
